...
bool isWordEqual(const char *word1, const char *word2) {
    // Periksa apakah panjangnya sama
    if (strlen(word1) != strlen(word2)) {
        return false;
    }

    // Bandingkan setiap karakter satu per satu
    for (int i = 0; word1[i] != '\0' && word2[i] != '\0'; i++) {
        if (word1[i] != word2[i]) {
            return false; // Jika ada perbedaan, tidak sama
        }
    }

    return true; // Jika semua karakter sama, berarti string sama
}
